...
$categoryCondition = " 1";
if (!empty($category)) {
	$categoryCondition = ' and cat_id = ' . $category;
	$count = 2;
}
...
//var_dump($_POST['searchx']); exit;
if ($search == 1000) {
	if (!empty($job) || !empty($category) || !empty($location)) {
		$sql = 'SELECT title FROM jobs where'. $jobCondition . $categoryCondition . $locationCondition;
	} else {
 		 ...
 		 <div class="row">
 		 	<div class="wrap-headline">
 		 		...
 		 		<div style="border-radius: 25px; background: #FFF; padding: 20px;">
 		 			<form class="form"  >
					<input type="hidden" name="searchx" value=""/>
